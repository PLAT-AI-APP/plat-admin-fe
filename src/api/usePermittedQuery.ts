 ...
 * **볼 수 없는 자료는 묻지도 않는다.**
 *
 * 서버가 403으로 막으면 자료는 새지 않는다. 그런데 화면은 운영자가 누르지도 않은 일에
 * 대해 거부 안내를 띄운다. 신고 화면이 대상 이름을 채우려고 캐릭터를 부르는데,
 * 캐릭터를 볼 수 없는 운영자에게는 화면을 열 때마다
 * "'캐릭터 > 조회' 권한이 필요합니다"가 뜬다. 누른 적이 없으니 고장으로 읽힌다.
 *
