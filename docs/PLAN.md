...
> **보관 문서다.** 최초 기능 범위를 정할 때 쓴 계획서로, 이후 결정(공식 캐릭터 → 공식 계정,
> 캐릭터 메뉴 → 세계관 메뉴, 금지어 등급 제거, Q&A · 신고의 MOCK 화면화 등)이 반영되어 있지 않다.
> 현재 계약은 `README.md`(연동 현황) · `docs/ADMIN_PLAN.md`(메뉴 · 화면) · plat-be `docs/`(API)를 본다.
