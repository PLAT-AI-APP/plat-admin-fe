...
`plat-fe`와 동일한 토큰 네이밍 체계를 쓴다. Tailwind v4 `@theme inline`으로
CSS 변수를 유틸리티 클래스에 매핑하므로, 화면에서는 **항상 시맨틱 클래스**를 쓴다.
...
파괴적 작업(삭제, 차단, 크레딧 차감)은 반드시 확인 다이얼로그를 거친다.

- `tone`: `danger` | `default`
- 본문에 **대상 이름을 명시**한다. ("`홍길동` 계정을 정지합니다.")
- 되돌릴 수 없는 작업은 경고 문구를 `text-font-error`로 추가한다.
- 확인 버튼 문구는 "삭제", "정지"처럼 **동작 이름**을 쓴다. "확인" 금지.
